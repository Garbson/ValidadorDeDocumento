    ...
    <div class="card-header">
      <div class="flex justify-between items-center">
        <h3 class="text-lg font-semibold flex items-center">
          ...
          Detalhes dos Erros ({{ filteredErrors.length }} de
          {{ errors.length }})
        </h3>
      ...
      <div class="overflow-x-auto">
        <table class="table">
          <thead class="table-header">
            <tr>
              <th
                class="table-header-cell cursor-pointer"
                @click="sortBy('linha')"
              >
                Linha
                <ChevronUp
                  v-if="sortField === 'linha' && sortDirection === 'asc'"
                  class="w-4 h-4 inline ml-1"
                />
                <ChevronDown
                  v-else-if="sortField === 'linha' && sortDirection === 'desc'"
                  class="w-4 h-4 inline ml-1"
                />
              </th>
              <th
              ...
              >
                Campo
                <ChevronUp
                  v-if="sortField === 'campo' && sortDirection === 'asc'"
                  class="w-4 h-4 inline ml-1"
                />
                <ChevronDown
                  v-else-if="sortField === 'campo' && sortDirection === 'desc'"
                  class="w-4 h-4 inline ml-1"
                />
              </th>
              <th
                class="table-header-cell cursor-pointer"
                @click="sortBy('erro_tipo')"
              >
                Tipo de Erro
                <ChevronUp
                  v-if="sortField === 'erro_tipo' && sortDirection === 'asc'"
                  class="w-4 h-4 inline ml-1"
                />
                <ChevronDown
                  v-else-if="
                    sortField === 'erro_tipo' && sortDirection === 'desc'
                  "
                  class="w-4 h-4 inline ml-1"
                />
              </th>
          ...
          <tbody class="table-body">
            ...
            >
              ...
              <td class="table-cell">
                <code
                  class="text-xs bg-gray-100 px-2 py-1 rounded max-w-xs truncate block"
                >
...
const filteredErrors = computed(() => {
  ...
  if (selectedErrorType.value) {
    filtered = filtered.filter(
      (error) => error.erro_tipo === selectedErrorType.value,
    );
  }
