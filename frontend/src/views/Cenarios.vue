<template>
  <div class="space-y-6">
    ...
    <div class="text-center">
      ...
      <p class="text-gray-600">Faça upload de um arquivo TXT para identificar os cenários de cada fatura</p>
    </div>
    ...
    <div v-if="isLoading" class="flex justify-center items-center py-12">
      <div class="text-center">
        ...
        <p class="text-gray-600">Analisando faturas e identificando cenários...</p>
      </div>
    </div>
    ...
    <div v-if="hasResults && !isLoading" class="space-y-6">
      ...
      <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
        ...
        <div class="card p-4 text-center">
          <p class="text-2xl font-bold text-purple-600">{{ faturasFiltradas.length }}</p>
          <p class="text-sm text-gray-600">Faturas Filtradas</p>
        </div>
        <div class="card p-4 text-center">
          <p class="text-2xl font-bold text-orange-600">{{ cenariosSelecionados.length || 'Todos' }}</p>
      ...
      <div class="card">
        <div class="card-header flex justify-between items-center">
          <h3 class="text-lg font-semibold">Selecione os Cenários para Filtrar</h3>
          <span v-if="cenariosSelecionados.length > 0" class="text-xs text-gray-500">
            Mostrando faturas que contenham <strong>todos</strong> os cenários selecionados
          </span>
        </div>
        <div class="card-body">
          <div class="flex flex-wrap gap-3">
            ...
            <button
              ...
              :class="cenariosSelecionados.length === 0
                ? 'border-blue-500 bg-blue-50 text-blue-700 shadow-md'
                : 'border-gray-200 bg-white text-gray-700 hover:border-blue-300 hover:bg-blue-50'"
            >
              <span class="block text-lg font-bold">{{ resultado.total_faturas }}</span>
              Todos
            </button>
            ...
            <button
              ...
              class="px-4 py-3 rounded-lg border-2 transition-all duration-200 text-sm font-medium"
              :class="cenariosSelecionados.includes(cenario)
                ? 'border-blue-500 bg-blue-50 text-blue-700 shadow-md'
                : 'border-gray-200 bg-white text-gray-700 hover:border-blue-300 hover:bg-blue-50'"
            >
              <span class="block text-lg font-bold">{{ resultado.contagem_por_cenario[cenario] }}</span>
              {{ cenario }}
            </button>
          </div>
        </div>
      ...
      <div class="card">
        ...
        <div class="card-body space-y-4">
          <p class="text-sm text-gray-500">
            Digite o código do campo (ex: <code class="bg-gray-100 px-1 rounded">01.02</code>) para buscar faturas por valor específico.
          </p>

          <div class="flex flex-col md:flex-row gap-4">
            <!-- Input do código do campo -->
            <div class="flex-1">
              <label class="block text-sm font-medium text-gray-700 mb-1">Código do Campo</label>
              <input
                v-model="campoBusca"
                type="text"
            ...
            <div v-if="campoEncontrado" class="flex-[2] bg-blue-50 border border-blue-200 rounded-lg p-3">
              <div class="grid grid-cols-2 md:grid-cols-4 gap-2 text-sm">
                <div>
                  <span class="text-gray-500">Nome:</span>
                  <span class="font-semibold text-blue-800 ml-1">{{ campoEncontrado.nome }}</span>
                </div>
                <div>
                ...
                <div>
                  ...
                  <span class="font-mono ml-1">{{ campoEncontrado.tamanho }}</span>
                </div>
                <div>
                  <span class="text-gray-500">Posição:</span>
                  <span class="font-mono ml-1">{{ campoEncontrado.posicao_de }}-{{ campoEncontrado.posicao_ate }}</span>
                </div>
              </div>
            </div>
            <div v-else-if="campoBusca.length >= 4 && !campoEncontrado" class="flex-[2] bg-red-50 border border-red-200 rounded-lg p-3 flex items-center">
              <span class="text-sm text-red-600">Campo não encontrado no layout.</span>
            </div>
          </div>

          <!-- Valor para buscar -->
          <div v-if="campoEncontrado" class="flex flex-col md:flex-row gap-4 items-end">
            <div class="flex-1">
          ...
          <div v-if="resultadoBuscaCampo" class="mt-4">
            <div class="bg-green-50 border border-green-200 rounded-lg p-3 mb-3 flex items-center justify-between">
              <span class="text-sm text-green-800">
                <strong>{{ resultadoBuscaCampo.total_encontradas }}</strong> fatura(s) encontrada(s) com
                <code class="bg-green-100 px-1 rounded">{{ campoBusca }}</code> = <code class="bg-green-100 px-1 rounded">"{{ valorBusca }}"</code>
              </span>
            </div>
          </div>
        </div>
      ...
      <div class="card">
        <div class="card-header flex justify-between items-center">
          <h3 class="text-lg font-semibold">
            Faturas
            <span v-if="cenariosSelecionados.length" class="text-blue-600">({{ cenariosSelecionados.join(' + ') }})</span>
            <span v-if="resultadoBuscaCampo" class="text-green-600 ml-2">
              | Campo {{ campoBusca }} = "{{ valorBusca }}"
            </span>
          </h3>
          <span class="text-sm text-gray-500">{{ faturasFiltradas.length }} faturas</span>
        </div>
        <div class="card-body p-0">
            ...
            <table class="min-w-full divide-y divide-gray-200">
              <thead class="bg-gray-50">
                <tr>
                  ...
                  <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Retenção</th>
                  <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Débito Auto.</th>
                  <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Tipos Registro</th>
                  <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Linha Início</th>
                  <th class="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Total Linhas</th>
                </tr>
              </thead>
              <tbody class="bg-white divide-y divide-gray-200">
                ...
                >
                  ...
                  <td class="px-4 py-3 text-sm font-mono">
                    <span v-if="fatura.aliquota_icms"
                      class="inline-block px-2 py-1 rounded-full text-xs font-bold"
                      :class="fatura.aliquota_icms === '0,00%'
                  ...
                  <td class="px-4 py-3 text-sm">
                    <div v-if="fatura.isencao" class="space-y-1">
                      <span class="inline-block px-2 py-1 rounded-full text-xs font-bold bg-teal-100 text-teal-800">
                        Isenta
                      </span>
                      <span v-if="fatura.valor_isentos" class="block text-xs font-mono text-teal-700 font-semibold">
                  ...
                  <td class="px-4 py-3 text-sm">
                    <div v-if="fatura.retencao" class="space-y-1">
                      <span class="inline-block px-2 py-1 rounded-full text-xs font-bold bg-red-100 text-red-800">
                        {{ fatura.retencao.percentual }}
                      </span>
                      <span class="block text-xs text-red-600">{{ fatura.retencao.tipo }}</span>
                      <span class="block text-xs font-mono text-red-700 font-semibold">
                        {{ fatura.retencao.valor }}
                      </span>
                      <!-- Detalhes PIS/CSSL/COFINS -->
                      <div v-if="fatura.retencao.detalhes" class="text-[10px] text-red-500 space-y-0.5 mt-1 border-t border-red-200 pt-1">
                        <div v-for="(info, nome) in fatura.retencao.detalhes" :key="nome">
                  ...
                  <td class="px-4 py-3 text-sm text-gray-600">{{ fatura.linha_inicio }}</td>
          ...
          <!-- Paginação -->
          <div v-if="totalPaginas > 1" class="flex items-center justify-between px-4 py-3 border-t border-gray-200">
            <div class="text-sm text-gray-500">
              Mostrando {{ (paginaAtual - 1) * itensPorPagina + 1 }} a {{ Math.min(paginaAtual * itensPorPagina, faturasFiltradas.length) }} de {{ faturasFiltradas.length }}
            </div>
...
import { Search, Loader2, RotateCcw, Filter, X } from 'lucide-vue-next'
import { identificarCenarios, getCamposLayout, buscarPorCampo } from '../services/api'
...
const faturasFiltradas = computed(() => {
  ...
  if (cenariosSelecionados.value.length === 0) return resultado.value.faturas
  return resultado.value.faturas.filter(f =>
    cenariosSelecionados.value.every(c => f.cenarios.includes(c))
  )
})
...
onMounted(async () => {
  try {
    ...
    console.warn('Layout PrintCenter não disponível para busca por campo:', e.message)
  }
})

function onCampoInput() {
  const codigo = campoBusca.value.trim()
  campoEncontrado.value = camposLayout.value.find(c => c.codigo === codigo) || null
...
async function executarBuscaCampo() {
  ...
  try {
    ...
    error.value = e.response?.data?.detail || e.message || 'Erro na busca por campo'
  } finally {
...
async function handleSubmit() {
  ...
  try {
    ...
    error.value = e.response?.data?.detail || e.message || 'Erro ao identificar cenários'
  } finally {
